...
function EarnTab({ user, userPoints, setUserPoints, setActiveView }: EarnTabProps) {
    ...
    const handleBackToList = () => {
        setView('list')
        // Force sync when returning from an activity
        if (typeof window !== 'undefined') {
            window.dispatchEvent(new Event('kojo:user:update'))
        }
    }
    ...
    if (view === 'offerwall' || view === 'africa-offerwalls') {
        return <AfricaOfferwallSystem userId={user?.id} onClose={handleBackToList} />
    }

    // Africa Surveys (Wannads, Adgate, Monlix) - Using survey APIs
    if (view === 'surveys') {
        return <AfricaSurveySystem userId={user?.id} onClose={handleBackToList} />
    }

    if (view === 'missions') {
        return <MissionSystem userId={user?.id} onClose={handleBackToList} />
    }

    if (view === 'referrals') {
        return <ReferralSystem user={user} onClose={handleBackToList} />
    }

    if (view === 'challenges') {
        return <DailyChallengeSystem userId={user?.id} onClose={handleBackToList} />
    }

    if (view === 'tournament') {
        return <TournamentSystem userId={user?.id} onClose={handleBackToList} />
    }

    if (view === 'games') {
        return <GameRewardSystem userId={user?.id} onClose={handleBackToList} />
    }
...
export default function EarnApp() {
    ...
    // Keep track of points for stale closures (intervals)
    const pointsRef = useRef(userPoints)
    useEffect(() => { pointsRef.current = userPoints }, [userPoints])
    ...
    const syncUserFromServer = async () => {
        try {
            ...
            // Remove AbortController for now to rule out compatibility issues
            // Add _t to prevent caching
            const res = await apiCall(`/api/user?userId=${encodeURIComponent(id)}&_t=${Date.now()}`)

            if (!res.ok) {
                console.error('[Sync] API Error:', res.status, res.statusText)
                // Only show error toast if it's a server error, to avoid spamming on network blips
                if (res.status >= 500) {
                    toast.error('Sync Error', { description: 'Please check your connection' })
                }
                return
            }
            ...
            if (data?.user) {
                const serverTotal = Number(data.user.totalPoints || 0)
                const serverPoints = Number(data.user.points || 0)
                const maxPoints = Math.max(serverTotal, serverPoints)

                console.log('[Sync] Server returned:', { total: serverTotal, points: serverPoints, using: maxPoints })

                // Only toast if there's a significant difference (e.g. points earned)
                if (maxPoints > pointsRef.current && pointsRef.current > 0) {
                    toast.success('Points Updated!', {
                        description: `New Balance: ${maxPoints.toLocaleString()}`,
                        duration: 3000
                    })
                }
                ...
                console.error('[Sync] Invalid data received:', data)
            }
        } catch (e: any) {
    ...
    useEffect(() => {
        ...
        // Auto-sync when app comes to foreground (e.g. returning from offerwall browser)
